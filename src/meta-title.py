...
def testOne(item, args):
    ...
    sumSpam = 0.0
    sumNoSpam = 0.0
    if args['mode'] == 'TitleOnly':
        titleRate = 1
        contentRate = 0
    elif args['mode'] == 'ContentOnly':
        titleRate = 0
        contentRate = 1
    else:
        titleRate = args['rate']
    ...
    for word in words:
        ...
        if args['mode'] == 'Split':
            ...
            freqSpam = x[0] * contentRate + x[2] * titleRate
            freqNoSpam = x[1] * contentRate + x[3] * titleRate

        if freqSpam == 0 or freqNoSpam == 0:
            sumSpam += log((freqSpam  + FLAT) * 1.0 / (spamTotal + FLAT * 2)) * contentRate
            sumNoSpam += log((freqNoSpam + FLAT) * 1.0 / (noSpamTotal + FLAT * 2)) * contentRate
        else:
            sumSpam += log(freqSpam * 1.0 / spamTotal) * contentRate
            sumNoSpam += log(freqNoSpam * 1.0 / noSpamTotal) * contentRate
...
if __name__ == '__main__':
    load()
    for mode in ['TitleOnly', 'ContentOnly', 'Add', 'Split']:
        if mode == 'TitleOnly' or mode == 'ContentOnly':
            args = {'mode': mode}
            result = testAll(args)
            printOne(result, args)
        else:
            for rate in [1, 10, 100]:
                args = {'mode': mode, 'rate': rate}
                result = testAll(args)
                printOne(result, args)
                if DEBUG:
                    break
