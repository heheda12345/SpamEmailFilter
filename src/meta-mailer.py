...
def getMailer(item):
    x = re.search("X-Mailer:(.*)", item['meta'])
    if x is not None:
        return x.group(1)
    else:
        return None
...
def init(items, args):
    ...
    spamMailer = Counter(spamMailer)
    noSpamMailer = Counter(noSpamMailer)
    ...
    global mailerList
    mailerList = {}
    for i, x in enumerate(spamMailer):
        mailerList[x] = [spamMailer[x], noSpamMailer[x]]
    for i, x in enumerate(noSpamMailer):
        mailerList[x] = [spamMailer[x], noSpamMailer[x]]
...
def testOne(item, args):
    ...
    mailer = getMailer(item)
    if mailer is not None:
        for word in split(mailer):
            if word not in mailerList:
                continue
            x = mailerList[word]
            freqSpam = x[0]
            freqNoSpam = x[1] 
            if freqSpam == 0 or freqNoSpam == 0:
                sumSpam += log((freqSpam  + FLAT) * 1.0 / (spamTotal + FLAT * 2)) * args['rate']
                sumNoSpam += log((freqNoSpam + FLAT) * 1.0 / (noSpamTotal + FLAT * 2)) * args['rate']
            else:
                sumSpam += log(freqSpam * 1.0 / spamTotal) * args['rate']
                sumNoSpam += log(freqNoSpam * 1.0 / noSpamTotal) * args['rate']

    return sumSpam - log(spamTotal) > sumNoSpam - log(noSpamTotal)

if __name__ == '__main__':
    load()
    for rate in [1, 4, 16, 64]:
        args = {'rate': rate}
        result = testAll(args)
